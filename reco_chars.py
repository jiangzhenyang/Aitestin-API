...
class CaffeCls(object):
    ...
    def _predict_cv2_imgs_sub(self, cv2_imgs, pos_start, pos_end):
        ...
        print '    net forwarding time: %f s' % (end-start)
        ...
        print '     geting result time: %f s' % (time.time()-start1)
...
def img2txt(caffe_cls,img):
    norm_width = 64
    norm_height = 64

    resize_keep_ratio = PreprocessResizeKeepRatio(1024, 1024)
    cv2_color_img = resize_keep_ratio.do(img)    
    ...
    for item in output_tag_to_max_proba:
        ocr_res += item[0][0]
    return ocr_res.encode("utf-8")

def pdf2txt(caffemodel,pdf_path,index):
    if not os.path.isdir('resultTexts'):
        os.mkdir('resultTexts')
    txtfile = open('resultTexts/'+str(index)+'.txt','w')

    image_pdf = Image(filename=pdf_path)
    image_jpeg = image_pdf.convert('jpeg')
    for i in range(1):
        img_page = Image(image=image_jpeg.sequence[i])
        img_buffer = np.asarray(bytearray(img_page.make_blob('jpeg')), dtype=np.uint8)
        img_need = cv2.imdecode(img_buffer, cv2.IMREAD_UNCHANGED)
        img_txt = img2txt(caffemodel,img_need)
        txtfile.write(img_txt)
    txtfile.close()
...
if __name__ == "__main__":
    ...
    for i in range(len(pdfs)):
        print 'recognizing number ' + str(i) + ' pdf ...'
	pdf2txt(caffe_cls,pdfs[i],i)
